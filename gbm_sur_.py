# -*- coding: utf-8 -*-
"""
优化后的网格搜索脚本（适配信用数据）- 特征自适应版本（特征名称问题修复版）
"""
import warnings
import sys
...
def load_data_with_validation():
    ...
    credit = read_csv_safe("sorteddataset_GBM_notime.csv")
    credit2 = read_csv_safe("filtered_data_GBM_notime.csv")
    credit3 = read_csv_safe("排序少样本_data_lgb.csv")
...
def safe_feature_select(df, features):
    ...
    return df.loc[:, features]  # 使用loc保证DataFrame结构


# 数据统一处理
X = safe_feature_select(credit.iloc[0:], FEATURE_ORDER)
y = credit.iloc[0:, -1]
X_train = safe_feature_select(credit2.iloc[:], FEATURE_ORDER)
y_train = credit2.iloc[:, -1]
X_out = safe_feature_select(credit3.iloc[0:], FEATURE_ORDER)
y_out = credit3.iloc[0:, -1]

# 修复点1：保持测试集为DataFrame
_, X_test, _, y_test = train_test_split(
    X, y,
    test_size=0.1,
    random_state=42
)
X_test = pd.DataFrame(X_test, columns=FEATURE_ORDER)  # 明确转换为DataFrame
...
def evaluate_model(model, X, y):
    # 修复点5：强制输入数据格式转换
    if not isinstance(X, pd.DataFrame):
        X = pd.DataFrame(X, columns=model.booster_.feature_name())

    # 特征顺序验证
    expected_features = model.booster_.feature_name()
    if list(X.columns) != expected_features:
        X = X[expected_features]  # 按模型特征顺序重排
...
if __name__ == "__main__":
    ...
    log_file = os.path.join(log_dir, f"auto_feature_gbm_3_{datetime.now().strftime('%Y%m%d%H%M')}.log")
