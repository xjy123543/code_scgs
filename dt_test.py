...
# 特征和标签提取
X = credit.iloc[0:1610, :15]  # 原始特征
y = credit.iloc[0:1610, -1]
X_train = credit2.iloc[0:1371, :15]  # 预处理后的训练特征
y_train = credit2.iloc[0:1371, -1]
X_out = credit3.iloc[0:, :15]  # 外部验证集
...
def rf_test_with_metrics_ci(model, X_data, y_data, data_type, threshold=0.5, n_bootstrap=1000, ci=0.95):
    ...
    # 可视化
    plot_roc_curve_with_ci('Decision Tree', bootstrap_y, bootstrap_probs, ci=ci)
    plot_calibration_curve_with_ci('Decision Tree', [y_data], [probas_all], ci=ci)

    plot_dca_with_ci('Decision Tree', bootstrap_y, bootstrap_probs, ci=ci)
...
if __name__ == "__main__":
    ...
    # 训练模型
    rf_model = train_random_forest(rf_params, X_train, y_train)
