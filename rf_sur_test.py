...
def train_random_forest(params, X_train, y_train, save_path='models'):
    ...
    with open('randomforest2.pkl', 'wb') as file:
...
def rf_test_with_metrics_ci(model, X_data, y_data, data_type, threshold=0.5, n_bootstrap=1000, ci=0.95):
    ...
    # 可视化
    plot_roc_curve_with_ci('Random Forest', bootstrap_y, bootstrap_probs, ci=ci)
    # plot_calibration_curve_with_ci('Random Forest', bootstrap_y, bootstrap_probs, ci=ci)

    plot_dca_with_ci('Random Forest', bootstrap_y, bootstrap_probs, ci=ci)
