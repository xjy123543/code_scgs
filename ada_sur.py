...
import time
import numpy as np
from sklearn.ensemble import AdaBoostClassifier  # 修改1：替换为AdaBoost
from sklearn.tree import DecisionTreeClassifier  # 修改2：添加基学习器
...
parameters = {
    'main_params': {
        'n_estimators': [50, 100,30,80],  # 弱分类器数量
        'learning_rate': [0.1,0.2,0.03,0.01,0.05],  # 学习率

    },
    'max_depth':[1,3,4, 5,31,15]
}
...
if __name__ == "__main__":
    ...
    log_file = os.path.join(log_dir, f"auto_feature_ada_3_{datetime.now().strftime('%Y%m%d%H%M')}.log")
    ...
    print("\n综合结果汇总:")
    print(summary.sort_values("Val AUC", ascending=False).head(3))
