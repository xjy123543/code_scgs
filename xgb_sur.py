...
# 数据统一处理
X = safe_feature_select(credit.iloc[0:1610], FEATURE_ORDER)
y = credit.iloc[0:1610, -1]
X_train = safe_feature_select(credit2.iloc[0:1371], FEATURE_ORDER)
y_train = credit2.iloc[0:1371, -1]
X_out = safe_feature_select(credit3.iloc[0:94], FEATURE_ORDER)
y_out = credit3.iloc[0:94, -1]
...
class DynamicFeatureSelector:
    def __init__(self, base_estimator, min_features=8):
        self.selector = RFECV(
...
parameters = {
    'main_params': {
        'max_depth': [3,  7,10,15,20,25],         # 更合理的树深度范围
        'n_estimators': [100,150,20,30,50,80],     # 树的数量
        'learning_rate': [0.01, 0.1,0.015,0.05],   # 学习率
...
if __name__ == "__main__":
    ...
    log_file = os.path.join(log_dir, f"auto_feature_xgb_{datetime.now().strftime('%Y%m%d%H%M')}.log")
